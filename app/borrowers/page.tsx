...
const getDaysOverdue = (loan: Loan): number => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [
          ...
          supabase.from("patrons").select("*").order("member_since", { ascending: false }),
          ...
          supabase.from("books").select("id, title, isbn, author").order("title"),
        ])
          ...
          if (simpleLoans) {
            ...
            const formattedLoans = simpleLoans.map((loan) => {
              ...
              return {
                ...
                borrower_name: patron?.full_name || `Unknown Patron (${loan.patron_id})`,
            ...
            console.log("📘 Fallback loans with names:", formattedLoans)
            setLoans(formattedLoans)
          } else {
            setLoans([])
          }
        } else {
          // ✅ Use joined data directly (and safely map names)
          const formattedLoans = (loansData || []).map((loan) => ({
            ...loan,
            borrower_name:
              loan.patrons?.full_name ||
              patronsData?.find((p) => p.id === loan.patron_id)?.full_name ||
              `Unknown Patron (${loan.patron_id})`,
            book_title:
              loan.books?.title ||
              booksData?.find((b) => b.id === loan.book_id)?.title ||
              `Unknown Book (${loan.book_id})`,
          }))

          console.log("📚 Joined loans with names:", formattedLoans)
          setLoans(formattedLoans)
        }
      } catch (error) {
  ...
  const activeLoans = enhancedLoans.filter((loan) => loan.status !== "returned" && !loan.returned_date)
  ...
  const returnedLoans = enhancedLoans.filter((loan) => loan.status === "returned" || loan.returned_date)
  ...
  const getActiveLoansByPatron = (patronId: string) => {
    return getLoansByPatron(patronId).filter((loan) => loan.status !== "returned" && !loan.returned_date)
  }
          ...
          <div className="max-w-7xl mx-auto space-y-4 sm:space-y-6">
            ...
            <div className="mb-3 sm:mb-4">
              ...
              <p className="text-xs sm:text-sm text-muted-foreground mt-1">
                Manage library patrons and track borrowed books
              </p>
            </div>
            ...
            <Card className="backdrop-blur-xl border-border/30 bg-gradient-to-b from-background/95 to-background/90 shadow-lg shadow-indigo-500/10">
              <CardHeader>
                ...
                <CardDescription className="text-xs sm:text-sm">Find patrons and track their loans</CardDescription>
              </CardHeader>
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-end gap-3 sm:gap-4">
                  ...
                  <div className="flex-1 relative w-full">
                    ...
                    <Input
                      ...
                      className="pl-11 bg-background/50 border-border/50 focus:border-indigo-300 transition-colors h-10 sm:h-11 w-full text-sm sm:text-base"
                    />
                  </div>

                  {/* Filter Dropdown */}
                  <div className="flex flex-col w-full sm:w-56">
                    <label className="text-xs sm:text-sm font-medium text-foreground/80 mb-1 flex items-center gap-2">
                      <Filter className="h-4 w-4 text-indigo-600" />
                      Status
                    </label>
                    <Select value={statusFilter} onValueChange={setStatusFilter}>
                      <SelectTrigger className="w-full bg-background/50 border-border/50 h-10 sm:h-11 text-sm sm:text-base">
            ...
            {/* Patron Detail Modal - CHANGE: responsive modal */}
                  ...
                  <CardHeader className="border-b border-border/30 p-4 sm:p-6">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
                      ...
                      <Button
                        ...
                        className="w-full sm:w-auto backdrop-blur-sm border-border/50 text-sm sm:text-base"
                      >
                        Close
                      </Button>
                    </div>
                  </CardHeader>
                  <CardContent className="p-4 sm:p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <p className="text-sm text-muted-foreground">
                                  {selectedPatron.phone || "Not provided"}
                                </p>
                              </div>
                            </div>
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <p className="text-sm text-muted-foreground">
                                  {formatDate(selectedPatron.member_since)}
                                </p>
                              </div>
                            </div>
                            <div className="pt-2">
                              <Badge
                                className={cn(
                                  ...
                                  statusColors[selectedPatron.status] || statusColors.inactive,
                                )}
                              >
                            ...
                            <div className="grid grid-cols-2 gap-4">
                              <div className="text-center p-3 bg-blue-50/50 rounded-lg backdrop-blur-sm border border-blue-200/50">
                                <div className="text-xl font-bold text-blue-600">
                                  {getLoansByPatron(selectedPatron.id).length}
                                </div>
                                    ...
                                    >
                                      <div className="flex-1">
                                        ...
                                        <p className="text-xs text-muted-foreground">
                                          Borrowed: {formatDate(loan.loan_date)} • Due:{" "}
                                  ...
                                  .map((loan) => (
                                    ...
                                    >
                                      <div className="flex-1">
                                        <p className="font-medium text-sm">{getBookTitle(loan)}</p>
                                        <p className="text-xs text-muted-foreground">
                                          Borrowed: {formatDate(loan.loan_date)} • Returned:{" "}
                                          {loan.returned_date ? formatDate(loan.returned_date) : "Unknown"}
                                        </p>
                                      </div>
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab}>
              <TabsList className="grid w-full grid-cols-3 backdrop-blur-sm bg-background/50 border-border/30 text-xs sm:text-sm">
                <TabsTrigger
                  value="borrowers"
                  className="flex items-center gap-1 sm:gap-2 data-[state=active]:bg-gradient-to-r data-[state=active]:from-indigo-500 data-[state=active]:to-purple-500 data-[state=active]:text-white text-xs sm:text-sm"
                >
                  <Users className="h-3 w-3 sm:h-4 sm:w-4" />
                  <span className="hidden sm:inline">Patrons</span>
                  <span className="sm:hidden">Patrons</span> ({filteredBorrowers.length})
                </TabsTrigger>
                <TabsTrigger
                  value="active-loans"
                  className="flex items-center gap-1 sm:gap-2 data-[state=active]:bg-gradient-to-r data-[state=active]:from-indigo-500 data-[state=active]:to-purple-500 data-[state=active]:text-white text-xs sm:text-sm"
                >
                  ...
                  <span className="hidden sm:inline">Active</span> ({activeLoans.length})
                </TabsTrigger>
                <TabsTrigger
                  value="overdue"
                  className="flex items-center gap-1 sm:gap-2 data-[state=active]:bg-gradient-to-r data-[state=active]:from-red-500 data-[state=active]:to-orange-500 data-[state=active]:text-white text-xs sm:text-sm"
                >
                  <AlertTriangle className="h-3 w-3 sm:h-4 sm:w-4" />
                  <span className="hidden sm:inline">Overdue</span> ({overdueLoans.length})
                </TabsTrigger>
              </TabsList>

              {/* Patrons Tab */}
              <TabsContent value="borrowers" className="space-y-4 sm:space-y-6">
                        ...
                        <CardHeader>
                          <div className="flex items-start justify-between">
                            ...
                            <Badge
                              className={cn(
                                "backdrop-blur-sm border",
                                statusColors[patron.status] || statusColors.inactive,
                              )}
                            >
                          ...
                          <CardDescription>Member since {formatDate(patron.member_since)}</CardDescription>
                        </CardHeader>
                        <CardContent>
                          <div className="space-y-3">
                            ...
                            {patronActiveLoans.length === 0 && patronReturnedLoans.length > 0 && (
                              <div className="text-center p-2 bg-gray-50/50 rounded-lg backdrop-blur-sm mt-2">
                                <p className="text-xs text-muted-foreground">
                                  {patronReturnedLoans.length} previous loan
                                  {patronReturnedLoans.length !== 1 ? "s" : ""}
                                </p>
                              </div>
                            )}
              ...
              {/* Active Loans Tab */}
              <TabsContent value="active-loans" className="space-y-4 sm:space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 lg:gap-6">
                    ...
                    return (
                      ...
                      >
                        <CardHeader>
                          ...
                          <CardTitle className="text-lg text-foreground line-clamp-2">{getBookTitle(loan)}</CardTitle>
                          <CardDescription className="line-clamp-1">Patron: {getPatronName(loan)}</CardDescription>
                        </CardHeader>
                        <CardContent>
                          <div className="space-y-3 text-sm">
                            ...
                            >
                              <div
                                className={cn("text-2xl font-bold", isLoanOverdue ? "text-red-600" : "text-blue-600")}
                              >
                  ...
                  {activeLoans.length === 0 && (
                    <div className="col-span-full text-center py-12">
                      ...
                      <p className="text-muted-foreground max-w-md mx-auto">
                        There are currently no active book loans. All books have been returned or no loans have been
                        created yet.
                      </p>
                    </div>
                  )}
                            ...
                            <CardContent>
                              <div className="space-y-3 text-sm">
                                ...
                                <div className="text-center p-3 bg-red-50/50 rounded-lg backdrop-blur-sm mt-2 border border-red-200/50">
                                  <div className="text-2xl font-bold text-red-600">{getDaysOverdue(loan)}</div>
